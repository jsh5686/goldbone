...
function dashboard() {
  ...
  const due = [...arr('complaints').filter((x) => x.processStatus !== '완료'), ...arr('consultations').filter((x) => x.status !== '회신완료')];
  view.innerHTML = `<div class="narrow">${pageHead('대시보드', new Date().toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' }), '<button id="print" class="btn">PDF 다운로드</button>')}<div class="grid stats"><button class="civil-card stat" data-go="/projects"><div class="icon blue">□</div><p>진행중 공사</p><strong>${active}</strong></button><button class="civil-card stat" data-go="/todos"><div class="icon green">☑</div><p>미완료 할 일</p><strong>${undone}</strong></button><button class="civil-card stat" data-go="/budgets"><div class="icon purple">₩</div><p>전체 예산</p><strong>${money(totalBudget)}</strong></button><button class="civil-card stat" data-go="/executions"><div class="icon orange">↧</div><p>집행액</p><strong>${money(executed)}</strong></button></div><div class="grid dash-grid"><section class="civil-card"><h3>기한 임박 항목 (D-7 이내)</h3>${due.length ? due.map((x) => `<p class="mini"><b>${esc(x.title)}</b><br>${date(x.dueDate)}</p>`).join('') : '<p class="mini">기한 임박 항목이 없습니다 ✓</p>'}</section><section class="civil-card"><h3>예산 집행 현황</h3>${arr('budgets').map((b) => { const total = arr('budgetParts').filter((p) => p.budgetId === b.id).reduce((a, p) => a + Number(p.amount || 0), 0); const used = arr('executions').filter((e) => e.budgetId === b.id).reduce((a, e) => a + Number(e.amount || 0), 0); return `<p class="mini"><b>${esc(b.name)}</b><br>${money(used)} / ${money(total)}</p><div class="progress"><span style="width:${total ? Math.round(used / total * 100) : 0}%"></span></div>`; }).join('') || '<p class="mini">등록된 예산이 없습니다</p>'}</section></div></div>`;
