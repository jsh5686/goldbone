...
function goFiltered(path, filters = {}) { pendingFilters[path] = filters; setRoute(path); }
function bindRouteLinks(root = document) {
  ...
  $$('[data-filter-route]', root).forEach((b) => b.onclick = () => {
    goFiltered(b.dataset.filterRoute, {
      projectId: nullableNumber(b.dataset.filterProject),
      budgetId: nullableNumber(b.dataset.filterBudget),
      scope: b.dataset.filterScope || '',
...
function simpleTable({ type, title, addText, placeholder, heads, fields, row, formFields, defaults = {}, filters = {} }) {
  const initial = pendingFilters[route] || {};
  const statusOptions = filters.statusField ? ['전체 상태', ...(filters.statusOptions || [...new Set(arr(type).map((x) => x[filters.statusField]).filter(Boolean))])] : [];
  view.innerHTML = `<div class="narrow">${pageHead(title, '', `<button class="btn" id="add-row">${addText}</button>`)}<div class="toolbar smart-toolbar"><div class="search"><input id="q" placeholder="${placeholder}"></div>${filters.project ? `<select id="filter-project" class="control-select">${projectOptions().map(([id, name]) => `<option value="${esc(id ?? '')}" ${String(id ?? '') === String(initial.projectId ?? '') ? 'selected' : ''}>${esc(id ? name : '전체 공사')}</option>`).join('')}</select>` : ''}${filters.budget ? `<select id="filter-budget" class="control-select">${budgetOptions().map(([id, name]) => `<option value="${esc(id ?? '')}" ${String(id ?? '') === String(initial.budgetId ?? '') ? 'selected' : ''}>${esc(name)}</option>`).join('')}</select>` : ''}${filters.statusField ? `<select id="filter-status" class="control-select">${statusOptions.map((name) => `<option value="${esc(name === '전체 상태' ? '' : name)}" ${String(initial.status || '') === String(name === '전체 상태' ? '' : name) ? 'selected' : ''}>${esc(name)}</option>`).join('')}</select>` : ''}${filters.dueField ? `<select id="filter-scope" class="control-select"><option value="" ${!initial.scope ? 'selected' : ''}>전체 기한</option><option value="open" ${initial.scope === 'open' ? 'selected' : ''}>미완료</option><option value="overdue" ${initial.scope === 'overdue' ? 'selected' : ''}>기한 초과</option><option value="week" ${initial.scope === 'week' ? 'selected' : ''}>이번 주</option></select>` : ''}<button id="clear-filters" class="btn ghost" type="button">필터 초기화</button></div><div class="table-card"><table class="table"><thead><tr>${heads.map((h) => `<th>${h}</th>`).join('')}<th>관리</th></tr></thead><tbody id="tbody"></tbody></table></div></div>`;
  ...
  const matchesScope = (x, scope) => {
    if (!filters.dueField || !scope) return true;
    const done = filters.doneField ? x[filters.doneField] === filters.doneValue : false;
    const remain = daysFromToday(x[filters.dueField]);
    if (scope === 'open') return !done;
    if (scope === 'overdue') return !done && remain !== null && remain < 0;
    if (scope === 'week') return remain !== null && remain >= 0 && remain <= 7;
    return true;
  };
  const rowText = (x) => `${fields.map((f) => x[f]).join(' ')} ${projectName(x.projectId)} ${budgetName(x.budgetId)} ${filters.statusField ? x[filters.statusField] : ''}`;
  const draw = () => {
    const q = $('#q').value.trim().toLowerCase();
    const projectId = filters.project ? nullableNumber($('#filter-project').value) : null;
    const budgetId = filters.budget ? nullableNumber($('#filter-budget').value) : null;
    const status = filters.statusField ? $('#filter-status').value : '';
    const scope = filters.dueField ? $('#filter-scope').value : '';
    const rows = arr(type)
      .filter((x) => projectId === null || x.projectId === projectId)
      .filter((x) => budgetId === null || x.budgetId === budgetId)
      .filter((x) => !status || x[filters.statusField] === status)
      .filter((x) => matchesScope(x, scope))
      .filter((x) => !q || rowText(x).toLowerCase().includes(q));
    $('#tbody').innerHTML = rows.map((x) => `<tr>${row(x).map((c) => `<td>${c}</td>`).join('')}<td>${rowActions(type, x.id)}</td></tr>`).join('') || `<tr><td colspan="${heads.length + 1}" class="muted">조건에 맞는 항목이 없습니다</td></tr>`;
    bindCrud(type, edit);
    bindRouteLinks($('#tbody'));
  };
  $('#add-row').onclick = () => edit();
  $('#q').oninput = draw;
  ['filter-project', 'filter-budget', 'filter-status', 'filter-scope'].forEach((id) => { const el = $(`#${id}`); if (el) el.onchange = draw; });
  $('#clear-filters').onclick = () => { pendingFilters[route] = {}; render(); };
  draw();
}
const projectOptions = () => [[null, '선택 안함'], ...arr('projects').map((p) => [p.id, p.name])];
const budgetOptions = () => [[null, '전체 예산'], ...arr('budgets').map((b) => [b.id, b.name])];
const projectScopedButton = (path, projectId, label, count, scope = '') => `<button class="scope-card" data-filter-route="${path}" data-filter-project="${projectId}" data-filter-scope="${scope}"><span>${label}</span><strong>${count}</strong></button>`;
function complaints() { simpleTable({ type: 'complaints', title: '민원관리', addText: '민원 등록', placeholder: '민원 제목, 민원인, 공사명 검색...', heads: ['구분', '제목', '관련 공사', '민원인', '접수일', '처리기한', '처리상태'], fields: ['title', 'complainant', 'phone', 'address', 'content', 'memo'], row: (x) => [esc(x.type), esc(x.title), projectLink(x.projectId), esc(x.complainant), date(x.receivedDate), date(x.dueDate), pill(x.processStatus, statusColor(x.processStatus))], defaults: { type: '전화민원', processStatus: '접수', receivedDate: today() }, filters: { project: true, statusField: 'processStatus', statusOptions: ['접수', '처리중', '완료'], dueField: 'dueDate', doneField: 'processStatus', doneValue: '완료' }, formFields: () => [{ name: 'type', label: '민원 유형' }, { name: 'processStatus', label: '처리 상태', type: 'select', options: ['접수', '처리중', '완료'].map((x) => [x, x]) }, { name: 'title', label: '민원 제목 *', required: true }, { name: 'complainant', label: '민원인' }, { name: 'phone', label: '연락처' }, { name: 'address', label: '주소' }, { name: 'receivedDate', label: '접수일', type: 'date' }, { name: 'dueDate', label: '처리기한', type: 'date' }, { name: 'projectId', label: '관련 공사', type: 'select', options: projectOptions() }, { name: 'content', label: '민원 내용', type: 'textarea' }, { name: 'memo', label: '처리 메모', type: 'textarea' }] }); }
function consultations() { simpleTable({ type: 'consultations', title: '협의관리', addText: '협의 등록', placeholder: '협의 제목, 기관명, 공사명 검색...', heads: ['구분', '협의 제목', '관련 공사', '협의 기관', '요청일', '처리기한', '회신일', '상태'], fields: ['title', 'department', 'manager', 'phone', 'content', 'result', 'memo'], row: (x) => [esc(x.type), esc(x.title), projectLink(x.projectId), esc(x.department), date(x.requestDate), date(x.dueDate), date(x.replyDate), pill(x.status, statusColor(x.status))], defaults: { type: '내부협의', status: '검토중', requestDate: today() }, filters: { project: true, statusField: 'status', statusOptions: ['검토중', '회신완료', '보류'], dueField: 'dueDate', doneField: 'status', doneValue: '회신완료' }, formFields: () => [{ name: 'type', label: '협의 유형' }, { name: 'status', label: '상태', type: 'select', options: ['검토중', '회신완료', '보류'].map((x) => [x, x]) }, { name: 'title', label: '협의 제목 *', required: true }, { name: 'department', label: '협의 기관' }, { name: 'manager', label: '담당자' }, { name: 'phone', label: '연락처' }, { name: 'requestDate', label: '요청일', type: 'date' }, { name: 'dueDate', label: '처리기한', type: 'date' }, { name: 'replyDate', label: '회신일', type: 'date' }, { name: 'projectId', label: '관련 공사', type: 'select', options: projectOptions() }, { name: 'content', label: '협의 내용', type: 'textarea' }, { name: 'result', label: '협의 결과', type: 'textarea' }, { name: 'memo', label: '메모', type: 'textarea' }] }); }
function budgets() {
  view.innerHTML = `<div class="narrow">${pageHead('예산관리', '', '<button id="add-budget" class="btn">예산 등록</button>')}<div class="cards">${arr('budgets').map((b) => { const parts = arr('budgetParts').filter((p) => p.budgetId === b.id); const total = parts.reduce((a, p) => a + Number(p.amount || 0), 0); const used = arr('executions').filter((e) => e.budgetId === b.id).reduce((a, e) => a + Number(e.amount || 0), 0); return `<article class="civil-card"><div class="item-top"><h3>${esc(b.name)}</h3><div>${rowActions('budgets', b.id)}</div></div><p class="mini">${esc(b.year || b.fiscalYear || '')} · ${esc(b.department || '')}</p><div class="field"><span>합계</span><strong>${money(total)}</strong></div><div class="field"><span>집행</span><strong>${money(used)}</strong></div><div class="progress"><span style="width:${total ? Math.round(used / total * 100) : 0}%"></span></div><div class="mini">${parts.map((p) => `${esc(p.source)} ${esc(p.carryover)} ${money(p.amount)} ${rowActions('budgetParts', p.id)}`).join('<br>') || '재원이 없습니다.'}</div><button class="btn outline" data-add-part="${b.id}">재원 추가</button> <button class="btn ghost" data-filter-route="/executions" data-filter-budget="${b.id}">집행 보기</button></article>`; }).join('')}</div></div>`;
  ...
  $('#add-budget').onclick = () => editBudget(); bindCrud('budgets', editBudget); bindCrud('budgetParts', editPart); $$('[data-add-part]').forEach((b) => b.onclick = () => editPart(null, Number(b.dataset.addPart))); bindRouteLinks(view);
}
function executions() { simpleTable({ type: 'executions', title: '집행관리', addText: '집행 등록', placeholder: '집행명, 업체, 공사명, 예산 검색...', heads: ['구분', '집행명', '관련 공사', '예산', '지출재원', '금액', '집행일', '업체', '상태'], fields: ['title', 'vendor', 'payee', 'memo', 'source', 'carryover'], row: (x) => [esc(x.type || x.execType), esc(x.title), projectLink(x.projectId), esc(budgetName(x.budgetId)), esc(x.source || x.carryover || '-'), money(x.amount), date(x.date || x.execDate), esc(x.vendor || x.payee || '-'), pill(x.status, statusColor(x.status))], defaults: { type: '지출', execType: '지출', status: '지출', date: today() }, filters: { project: true, budget: true, statusField: 'status', dueField: 'date' }, formFields: () => [{ name: 'type', label: '집행 유형' }, { name: 'status', label: '상태' }, { name: 'title', label: '집행명 *', required: true }, { name: 'budgetId', label: '예산', type: 'select', options: [[null, '선택 안함'], ...arr('budgets').map((b) => [b.id, b.name])] }, { name: 'projectId', label: '관련 공사', type: 'select', options: projectOptions() }, { name: 'date', label: '집행일', type: 'date' }, { name: 'vendor', label: '업체/수취인' }, { name: 'source', label: '지출재원' }, { name: 'carryover', label: '이월구분' }, { name: 'amount', label: '총 금액 (원)', type: 'number' }, { name: 'memo', label: '메모', type: 'textarea' }] }); }
function schedules() {
  const initial = pendingFilters[route] || {};
  const filterProjectId = nullableNumber(initial.projectId);
  ...
  const filteredSchedules = arr('schedules').filter((x) => filterProjectId === null || x.projectId === filterProjectId);
  view.innerHTML = `<div class="narrow">${pageHead('일정관리', '', '<button id="add-schedule" class="btn">일정 등록</button>')}<div class="toolbar smart-toolbar"><button id="prev-month" class="btn outline">‹</button><h2>${y}년 ${m + 1}월</h2><button id="next-month" class="btn outline">›</button><button id="this-month" class="btn ghost">오늘</button><select id="filter-project" class="control-select">${projectOptions().map(([id, name]) => `<option value="${esc(id ?? '')}" ${String(id ?? '') === String(filterProjectId ?? '') ? 'selected' : ''}>${esc(id ? name : '전체 공사')}</option>`).join('')}</select><button id="clear-filters" class="btn ghost" type="button">필터 초기화</button></div><div class="calendar"><div class="cal-head">${['일', '월', '화', '수', '목', '금', '토'].map((x) => `<div>${x}</div>`).join('')}</div><div class="cal-row">${cells.map((d) => { const ev = d ? filteredSchedules.filter((s) => new Date(s.date || s.startDate).getMonth() === m && new Date(s.date || s.startDate).getDate() === d && new Date(s.date || s.startDate).getFullYear() === y) : []; return `<div class="day">${d ? `<div class="day-num">${d}</div>${ev.map((e) => `<button class="event" data-schedule="${e.id}">${esc(e.title)}</button>`).join('')}` : ''}</div>`; }).join('')}</div></div><div class="section table-card"><table class="table"><thead><tr><th>일자</th><th>유형</th><th>일정 제목</th><th>관련 공사</th><th>장소</th><th>관리</th></tr></thead><tbody>${filteredSchedules.filter((x) => new Date(x.date || x.startDate).getFullYear() === y && new Date(x.date || x.startDate).getMonth() === m).map((x) => `<tr><td>${date(x.date || x.startDate)}</td><td>${esc(x.type || '')}</td><td>${esc(x.title)}</td><td>${projectLink(x.projectId)}</td><td>${esc(x.location || '')}</td><td>${rowActions('schedules', x.id)}</td></tr>`).join('') || '<tr><td colspan="6" class="muted">조건에 맞는 일정이 없습니다</td></tr>'}</tbody></table></div></div>`;
  ...
  $('#add-schedule').onclick = () => editSchedule(); $('#prev-month').onclick = () => { currentMonth = new Date(y, m - 1, 1); render(); }; $('#next-month').onclick = () => { currentMonth = new Date(y, m + 1, 1); render(); }; $('#this-month').onclick = () => { currentMonth = new Date(); render(); }; $('#filter-project').onchange = (e) => { pendingFilters[route] = { projectId: nullableNumber(e.target.value) }; render(); }; $('#clear-filters').onclick = () => { pendingFilters[route] = {}; render(); }; $$('[data-schedule]').forEach((b) => b.onclick = () => editSchedule(Number(b.dataset.schedule))); bindCrud('schedules', editSchedule); bindRouteLinks(view);
}
...
function todos() {
  ...
  const doneRows = topRows.filter((row) => row.item.done);
  const openRows = topRows.filter((row) => !row.item.done);
  ...
  const drawList = () => {
    ...
    const [kicker, heading] = viewLabels[scope] || viewLabels.today;
    $('#todo-list-kicker').textContent = kicker;
    $('#todo-list-heading').textContent = heading;
    $('#todo-list-count').textContent = `${filtered.length}건`;
    $('#todo-action-list').innerHTML = filtered.map(todoActionRow).join('') || '<div class="empty-state">조건에 맞는 할 일이 없습니다.</div>';
...
function files() {
  const initial = pendingFilters[route] || {};
  const projectId = nullableNumber(initial.projectId);
  const rows = arr('files').filter((f) => projectId === null || f.projectId === projectId);
  view.innerHTML = `<div class="narrow">${pageHead('자료실', `총 ${rows.length}개 파일`, '<button id="add-file" class="btn">파일 업로드</button>')}<div class="toolbar smart-toolbar"><select id="filter-project" class="control-select">${projectOptions().map(([id, name]) => `<option value="${esc(id ?? '')}" ${String(id ?? '') === String(projectId ?? '') ? 'selected' : ''}>${esc(id ? name : '전체 공사')}</option>`).join('')}</select><button id="clear-filters" class="btn ghost" type="button">필터 초기화</button></div><div class="cards">${rows.map((f) => `<article class="civil-card"><h3>${esc(f.file?.name || f.fileName || f.name)}</h3><p class="mini">${esc(f.category || '기타')} · ${projectLink(f.projectId)}<br>${esc(f.description || '')}<br>${date(f.createdAt || f.uploadedAt)}</p><button class="link-btn" data-download-file="${f.id}">다운로드</button> ${rowActions('files', f.id)}</article>`).join('') || '<div class="empty civil-card">조건에 맞는 파일이 없습니다.</div>'}</div></div>`;
  $('#add-file').onclick = () => editFile(null, projectId); $('#filter-project').onchange = (e) => { pendingFilters[route] = { projectId: nullableNumber(e.target.value) }; render(); }; $('#clear-filters').onclick = () => { pendingFilters[route] = {}; render(); }; bindCrud('files', editFile); $$('[data-download-file]').forEach((b) => b.onclick = () => downloadFile(Number(b.dataset.downloadFile))); bindRouteLinks(view);
}
